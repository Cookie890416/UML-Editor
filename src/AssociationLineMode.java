...
public class AssociationLineMode extends Mode{
	...
	public void mouseReleased(MouseEvent arg0){
		...
		if(selected_object_number != null){
			...
			//如果end_selected_object_number不是null，則建立AssociationLine物件並將它加入到UMLCanvas的物件Map中
			if(end_selected_object_number != null){
				AssociationLine association_line = new AssociationLine(Configure.AssociationLineBut_title, umlObjectMap.get(selected_object_number), umlObjectMap.get(end_selected_object_number));
				UMLCanvas.getInstance().addObjectToMap(association_line);
			}
		}
		//重繪UMLCanvas
	...
	public void drawDraggedLine(Graphics2D g2) {
		if(selected_object_number != null){
			...
			//劃一條association line
			g2.drawLine((int)line_start_point.getX(), (int)line_start_point.getY(), (int)dragged_point.getX(), (int)dragged_point.getY());
